...
jQuery.getJSON(GetQueryStringParams("config",configFile), function(data, textStatus, jqXHR) {
	config=data;
...
function setupGUI(config) {
	...
	if (config.logo.file) {
		...
		logo = "<img style=\"display: block; margin: -30px auto; height: auto; width: 90%\" src=\"" + config.logo.file +"\"";
...
function nodeActive(a) {
    ...
    d = "";
		for (g in e) {
            ...
            //f.push('<li class="membership"><a href="#' + c.name + '" onmouseover="\" onclick=\"nodeActive(\'' + c.id + '\')" onmouseout="">' + c.name + "</a></li>");
    ...
    if (f.attributes) {
        ...
        if (image_attribute) {
            ...
            // $GP.info_name.html("<div><span onmouseover=\"\" onmouseout=\"\">" + b.label + "</span></div>");
            // sigInst._core.plotter.drawHoverNode(sigInst._core.graph.nodesIndex["" + b.id + ""]);
        }
    ...
    // $GP.info_data.show();
    $GP.info_name.click(function () { $GP.info_data.toggle(); });
    $GP.info_data.hide();
...
function showCluster(a) {
    ...
    if (b && 0 < b.length) {
        ...
        for (var f = [], e = [], c = 0, g = b.length; c < g; c++) {
            ...
            // !0 == d.hidden && (e.push(b[c]), d.hidden = !1, d.attr.lineWidth = !1, d.attr.color = d.color, f.push('<li class="membership"><a href="#'+d.label+"\" onclick=\"nodeActive(" + d.id + ')" >' + d.label + "</a></li>"), sigInst._core.plotter.drawHoverNode(sigInst._core.graph.nodesIndex[\'' + d.id + "']))
        }
